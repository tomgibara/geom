...
public final class BezierCurve extends Curve {
	...
	BezierCurve(List<Point> points, double[] coords) {
	...
	private BezierCurve(Point from, Point to) {
		...
		coords = new double[] { from.x, from.y, to.x, to.y };
		order = 1;
	}

	private BezierCurve(Point only) {
		points = Collections.singletonList(only);
		coords = new double[] { only.x, only.y };
		order = 0;
	}
	...
	public Point pointAt(double t) {
        ...
        return switch (order) {
            case 0 -> points.get(0);
            case 1 -> Point.Util.interpolate(points.get(0), points.get(1), t);
            default -> {
                double[] fs = computeCasteljau(t, coords);
                yield new Point(fs[0], fs[1]);
            }
        };
	}
	...
	public Vector tangentAt(double t) {
		...
		default:
			t = clamp(t);
			if (t == 0.0) return points.get(1).vectorFrom(points.get(0)).normalized();
			if (t == 1.0) return points.get(order).vectorFrom(points.get(order - 1)).normalized();
			double[] fs = computeCasteljau(clamp(t), coords);
			return new Vector(fs[2] - fs[0], fs[3] - fs[1]).normalized();
		}
	}

	@Override
	public PointPath pointTangentAt(double t) {
		if (order == 0) {
	...
	public double magnitudeAt(double t) {
		Norm norm = Norm.L2;
	...
	@Override
	protected BezierCurve computeSegment(double minP, double maxP) {
		BezierCurve tail = fromPoints(computeCasteljau(minP, coords).clone(), true);
		BezierCurve head = fromPoints(computeCasteljau((maxP - minP)/(1.0 - minP), tail.coords).clone(), false);
		return head;
	}
	...
	protected Curve computeDerivative() {
		...
		double[] cs = new double[order * 2];
		double[] coords = this.coords;
		for (int i = 0; i < cs.length;) {
			double ax = coords[i    ];
			double ay = coords[i + 1];
			double bx = coords[i + 2];
			double by = coords[i + 3];
			double x = (bx - ax) * order;
			double y = (by - ay) * order;
			cs[i++] = x;
			cs[i++] = y;
	...
	// returns working doubles
	private static double[] computeCasteljau(double t, double[] coords) {
		final double s = 1 - t;
		int length = coords.length;
		double[] fs = GeomUtil.workingFloats(length * 2);
	...
	private BezierCurve curveFrom(double t, double[] fs) {
		if (t <= 0) return this;
	...
	private BezierCurve curveTo(double t, double[] fs) {
	...
	private BezierCurve fromPoints(double[] fs, boolean from) {
	...
	private static final class BezierPath extends CurvePath {
		...
		public <K> K linearize(Point.Consumer<K> consumer) {
			...
			default:
				SplitCurvePath curves = ((BezierCurve)z).splitAt(0.5);
		...
		public boolean equals(Object obj) {
			...
			if (!(obj instanceof BezierPath that)) return false;
